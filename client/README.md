...
Type-safe fetch client that generates TypeScript types from static [OpenAPI](https://spec.openapis.org/oas/latest.html) schemas using Node.js. It ensures type safety when consuming the .NET API. The  **openapi.json** file is created during the build phase of `DNE.Todo.API` and is located at[/service/DNE.Todo.API/bin/Debug/net8.0/openapi.json](/service/DNE.Todo.API/bin/Debug/net8.0/openapi.json). The `yarn generate-api` command then generates [src/api/todo-schema.d.ts](src/api/todo-schema.d.ts) from this file.
...
For example, the configuration file [src/config/config.ts](client/src/config/config.ts) validates the `CONFIG` variable, which is derived from the environment variables using a Zod schema.
...
This folder contains all the stuff related to 