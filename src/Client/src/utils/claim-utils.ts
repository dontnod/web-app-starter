...
export type IdTokenClaims = TokenClaims &
  Record<string, string | number | string[] | object | undefined | unknown>
...
export const createClaimsTable = (claims: IdTokenClaims) => {
  const claimsObj: Record<keyof IdTokenClaims, ClaimsData> = {}
  let index = 0

  Object.keys(claims).forEach((untypedKey) => {
    const key = untypedKey as keyof IdTokenClaims
    ...
    switch (key) {
      ...
      case '_claim_sources':
      default:
        populateClaim(key.toString(), claims[key] as string, '', index, claimsObj)
        index++
    }
  })
...
const populateClaim = (
  ...
  description: string,
  index: number,
  claimsObject: Record<string | number, ClaimsData>
) => {
